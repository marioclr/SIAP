...
Const L_PAPERWORK_START_DATE_FLAGS = 151
Const L_PAPERWORK_ESTIMATED_DATE_FLAGS = 152
Const L_PAPERWORK_END_DATE_FLAGS = 153
Const L_PAPERWORK_DOCUMENT_NUMBER_FLAGS = 154
Const L_PAPERWORK_TYPE_FLAGS = 155
Const L_PAPERWORK_OWNER_FLAGS = 156
Const L_PAPERWORK_STATUS_FLAGS = 157
Const L_PAPERWORK_PRIORITY_FLAGS = 158
Const L_PAPERWORK_OWNERS_FLAGS = 159
...
'************************************************************
	...
	If (lErrorNumber = 0) And ((InStr(1, sFlags, ("," & L_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_OPEN_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_CLOSED_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_PAYROLL1_FLAGS & ",")) > 0)) Then
		If bFromRequest Then
...
'			sFilter = sFilter & DisplayLikeText(CInt(sLike)) & sID & "<BR />"
	...
	If (lErrorNumber = 0) And (InStr(1, sFlags, ("," & L_PAYMENT_CENTER_FLAGS & ",")) > 0) Then
		...
		If Len(sID) > 0 Then
			Call GetNameFromTable(oADODBConnection, "PaymentCenters", sID, "&nbsp;&nbsp;&nbsp;-", "<BR />", sNames, "")
			sFilter = sFilter & sNames & "<BR />"
...
'************************************************************
	...
	If InStr(1, sFlags, ("," & L_ZIP_WARNING_FLAGS & ",")) > 0 Then Response.Write "<DIV NAME=""ReportFilterDiv"" ID=""ReportFilterDiv"">"
		...
		If InStr(1, sFlags, ("," & L_NO_DIV_FLAGS & ",")) = 0 Then Response.Write "<DIV CLASS=""ReportFilter"">"
			If (lErrorNumber = 0) And ((InStr(1, sFlags, ("," & L_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_OPEN_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_CLOSED_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_PAYROLL1_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_ORDINARY_PAYROLL_FLAGS & ",")) > 0)) Then
				Response.Write "<SCRIPT LANGUAGE=""JavaScript""><!--" & vbNewLine
					Response.Write "var aPayrolls = new Array("
						Call GenerateJavaScriptArrayFromQuery(oADODBConnection, "Payrolls", "PayrollID", "PayrollTypeID", "(IsClosed<>1)", "PayrollID", sErrorDescription)
...
'							Response.Write "&nbsp;<A HREF=""javascript: RemoveSelectedItemsFromList(null, document.ReportFrm.EmployeeIDs); SelectAllItemsFromList(document.ReportFrm.EmployeeIDs);""><IMG SRC=""Images/BtnCrclDelete.gif"" WIDTH=""16"" HEIGHT=""16"" ALT=""Quitar"" BORDER=""0""></A><BR />"
			...
			If (lErrorNumber = 0) And (InStr(1, sFlags, ("," & S_CREDITS_UPLOADED_FILE_NAME & ",")) > 0) Then
				...
				Response.Write "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<SELECT NAME=""UploadedFileName"" ID=""UploadedFileNameLst"" SIZE=""1"" CLASS=""Lists"">"
					Response.Write "<OPTION VALUE="""">Todos</OPTION>"
					Response.Write GenerateListOptionsFromQuery(oADODBConnection, "Credits", "Distinct UploadedFileName", "UploadedFileName As UploadedFileName2", "UploadedFileName IS NOT NULL AND UploadedFileName <> ''", "UploadedFileName", "", "", sErrorDescription)
				Response.Write "</SELECT><BR /><BR />"
				iCounter = iCounter + 1
			End If
			If (lErrorNumber = 0) And (InStr(1, sFlags, ("," & L_ABSENCE_ID_FLAGS & ",")) > 0) Then
		...
		Response.Write "<SCRIPT LANGUAGE=""JavaSCript""><!--" & vbNewLine
			...
			Response.Write "if (document.ReportFrm.DisasterID) {" & vbNewLine
				...
				Next
			Response.Write "}" & vbNewLine
			If (StrComp(GetASPFileName(""), "Payroll.asp", vbBinaryCompare) = 0) And ((InStr(1, sFlags, ("," & L_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_OPEN_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_CLOSED_PAYROLL_FLAGS & ",")) > 0) Or (InStr(1, sFlags, ("," & L_PAYROLL1_FLAGS & ",")) > 0)) Then
				Response.Write "DisplayPayrollFilters(document.ReportFrm.PayrollID.value);" & vbNewLine
			End If
		Response.Write "//--></SCRIPT>" & vbNewLine
...
'************************************************************
	...
	ElseIf Len(oRequest("EmployeeNumbers").Item) > 0 Then
		...
		If UBound(asTemp) > 990 Then
			...
			lErrorNumber = ExecuteSQLQuery(oADODBConnection, "DELETE FROM " & sTableName , "ReportsLib.asp", S_FUNCTION_NAME, 000, sErrorDescription, Null)
			...
			Next
			sCondition = sCondition & " And (Employees.EmployeeID = (Select Distinct EmployeeID From " & sTableName & " Where (Employees.EmployeeID=EmployeeID)))"
		Else
...
'************************************************************
	...
	Select Case CLng(lReportNumber)
		...
		Case JOBS_LIST_REPORTS, SPECIAL_JOBS_LIST_REPORTS
			If StrComp(oRequest("JobStatusID").Item, "1", vbBinaryCompare) = 0 Then
				GetReportPathByConstant = "<A HREF=""Main_ISSSTE.asp?SectionID=1"">Personal</A> > <A HREF=""Jobs.asp"">Plazas</A> > <B>Plazas ocupadas</B>"
			ElseIf StrComp(oRequest("JobStatusID").Item, "2", vbBinaryCompare) = 0 Then
				GetReportPathByConstant = "<A HREF=""Main_ISSSTE.asp?SectionID=1"">Personal</A> > <A HREF=""Jobs.asp"">Plazas</A> > <B>Plazas vacantes</B>"
			ElseIf StrComp(oRequest("JobStatusID").Item, "3", vbBinaryCompare) = 0 Then
				GetReportPathByConstant = "<A HREF=""Main_ISSSTE.asp?SectionID=1"">Personal</A> > <A HREF=""Jobs.asp"">Plazas</A> > <B>Plazas congeladas</B>"
			ElseIf StrComp(oRequest("JobStatusID").Item, "4", vbBinaryCompare) = 0 Then
				GetReportPathByConstant = "<A HREF=""Main_ISSSTE.asp?SectionID=1"">Personal</A> > <A HREF=""Jobs.asp"">Plazas</A> > <B>Plazas con licencia</B>"
			ElseIf Len(oRequest("MovedEmployees").Item) > 0 Then
				GetReportPathByConstant = "<A HREF=""Main_ISSSTE.asp?SectionID=1"">Personal</A> > <A HREF=""Jobs.asp"">Plazas</A> > <B>Plazas en interinatos</B>"
			ElseIf Len(oRequest("JobsOwners").Item) > 0 Then
				GetReportPathByConstant = "<A HREF=""Main_ISSSTE.asp?SectionID=1"">Personal</A> > <A HREF=""Jobs.asp"">Plazas</A> > <B>Plazas reservadas</B>"
			Else
				GetReportPathByConstant = "<A HREF=""Main_ISSSTE.asp?SectionID=1"">Personal</A> > <A HREF=""Jobs.asp"">Plazas</A> > <B>Reporte de plazas por estatus</B>"
			End If
		Case EMPLOYEE_HISTORY_LIST_REPORTS
