	...
	Else
		...
		If lErrorNumber = 0 Then
			If oRecordset.EOF Then
				...
				oRecordset.Close
			Else
				If (aConceptComponent(N_ID_CONCEPT) = CLng(oRecordset.Fields("ConceptID").Value)) And _
				(aConceptComponent(N_COMPANY_ID_CONCEPT) = CLng(oRecordset.Fields("CompanyID").Value)) And _
...
'************************************************************
	...
	Else
		...
		sErrorDescription = "No se pudo guardar la información del nuevo registro."
		If Not bHasChanged Then
			lErrorNumber = ExecuteSQLQuery(oADODBConnection, "Update ConceptsValues Set ConceptAmount=" & aConceptComponent(D_CONCEPT_AMOUNT_CONCEPT) & ", StartDate=" & aConceptComponent(N_START_DATE_FOR_VALUE_CONCEPT) & ", EndDate=" & aConceptComponent(N_END_DATE_FOR_VALUE_CONCEPT) & " Where (RecordID=" & aConceptComponent(N_RECORD_ID_CONCEPT) & ")", "ConceptComponent.asp", S_FUNCTION_NAME, 000, sErrorDescription, Null)
		Else
...
'************************************************************
	...
	Else
		...
		If lErrorNumber = 0 Then
			If oRecordset.EOF Then
				sErrorDescription = "No se pudo revisar la existencia del registro en la base de datos."
				Call DisplayTimeStamp("Select * From Positions Where (CompanyID=" & aConceptComponent(N_COMPANY_ID_CONCEPT) & ") And (ClassificationID=" & aConceptComponent(N_CLASSIFICATION_ID_CONCEPT) & ") And (GroupGradeLevelID=" & aConceptComponent(N_GROUP_GRADE_LEVEL_ID_CONCEPT) & ") And (IntegrationID=" & aConceptComponent(N_INTEGRATION_ID_CONCEPT) & ") And (PositionShortName ='" & sPositionShortName & "')  And (LevelID=" & aConceptComponent(N_LEVEL_ID_CONCEPT) & ") And (EmployeeTypeID=" & aConceptComponent(N_EMPLOYEE_TYPE_ID_CONCEPT) & ") And (Workinghours=" & aConceptComponent(D_WORKING_HOURS_CONCEPT) & ") And (((StartDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (EndDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")) Or ((EndDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (EndDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")) Or ((EndDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (StartDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")))")
				lErrorNumber = ExecuteSQLQuery(oADODBConnection, "Select * From Positions Where (CompanyID=" & aConceptComponent(N_COMPANY_ID_CONCEPT) & ") And (ClassificationID=" & aConceptComponent(N_CLASSIFICATION_ID_CONCEPT) & ") And (GroupGradeLevelID=" & aConceptComponent(N_GROUP_GRADE_LEVEL_ID_CONCEPT) & ") And (IntegrationID=" & aConceptComponent(N_INTEGRATION_ID_CONCEPT) & ") And (PositionShortName ='" & sPositionShortName & "')  And (LevelID=" & aConceptComponent(N_LEVEL_ID_CONCEPT) & ") And (((StartDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (EndDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")) Or ((EndDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (EndDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")) Or ((EndDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (StartDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")))", "ConceptComponent.asp", S_FUNCTION_NAME, 000, sErrorDescription, oRecordset)
				If lErrorNumber = 0 Then
					If oRecordset.EOF Then
						lErrorNumber = L_ERR_NO_RECORDS
						sErrorDescription = "El puesto " & sPositionShortName & ", con los parámetros indicados no se encuentra en a base de datos."
						Call DisplayTimeStamp("Select * From Positions Where (CompanyID=" & aConceptComponent(N_COMPANY_ID_CONCEPT) & ") And (ClassificationID=" & aConceptComponent(N_CLASSIFICATION_ID_CONCEPT) & ") And (GroupGradeLevelID=" & aConceptComponent(N_GROUP_GRADE_LEVEL_ID_CONCEPT) & ") And (IntegrationID=" & aConceptComponent(N_INTEGRATION_ID_CONCEPT) & ") And (PositionShortName ='" & sPositionShortName & "')  And (LevelID=" & aConceptComponent(N_LEVEL_ID_CONCEPT) & ") And (((StartDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (EndDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")) Or ((EndDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (EndDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")) Or ((EndDate>=" & aConceptComponent(N_START_DATE_CONCEPT) & ") And (StartDate<=" & aConceptComponent(N_END_DATE_CONCEPT) & ")))")
					Else
						If CLng(oRecordset.Fields("EndDate").Value) < CLng(aConceptComponent(N_START_DATE_FOR_REGISTRATION_CONCEPT)) Then
							...
							Call DisplayTimeStamp(sErrorDescription)
						Else
							aConceptComponent(N_POSITION_ID_CONCEPT) = CLng(oRecordset.Fields("PositionID").Value)
						End If
					End If
...
'************************************************************
	...
	End Select
	If Len(oRequest("ChangeEndDate").Item) = 0 Then
		If bFull Then
			If Len(oRequest("RecordID").Item) > 0 Then
				'lErrorNumber = GetConcept(oRequest, oADODBConnection, aConceptComponent, sErrorDescription)
	...
	If lErrorNumber = 0 Then
		Response.Write "<SCRIPT LANGUAGE=""JavaScript""><!--" & vbNewLine
			...
			Response.Write "var aLevelID = new Array("
				Call GenerateJavaScriptArrayFromQuery(oADODBConnection, "Positions, Levels", "Positions.LevelID", "PositionShortName, LevelShortName, LevelName", "(Positions.LevelID=Levels.LevelID) And (Levels.EndDate=30000000) And (Levels.Active=1)", "PositionShortName, Positions.StartDate", sErrorDescription)
				...
				Response.Write "if (oForm) {" & vbNewLine
                    ...
                    If bFull Then
						...
						Response.Write "if (! CheckFloatValue(oForm.ConceptMax, 'el monto máximo', N_MINIMUM_ONLY_FLAG, N_CLOSED_FLAG, 0, 0))" & vbNewLine
							Response.Write "return false;" & vbNewLine
                    Else
					    Response.Write "if(oForm.PositionID.value<0){" &vbNewLine
						    Response.Write "alert('Favor de seleccionar el puesto a tabular');" &vbNewLine
			...
			Response.Write "function DisplayInfoMessage() {" & vbNewLine
				...
				'Response.Write "}" & vbNewLine
				Response.Write "alert('Nivel ' + document.ConceptValuesFrm.LevelID1.value)" & vbNewLine
			Response.Write "} // End of DisplayInfoMessage" & vbNewLine


			Response.Write "function UpdateLevels(sTypeID) {" & vbNewLine
				Response.Write "oForm = document.ConceptValuesFrm;" & vbNewLine

				Response.Write "if (oForm) {" & vbNewLine
					Response.Write "RemoveAllItemsFromList(null, oForm.LevelID);" & vbNewLine
					Response.Write "for (var i=0; i<aLevelID.length; i++)" & vbNewLine
						Response.Write "if (aLevelID[i][1] == sTypeID)" & vbNewLine
							Response.Write "AddItemToList(aLevelID[i][2] + '. ' + aLevelID[i][3], aLevelID[i][0], null, oForm.LevelID);" & vbNewLine
				Response.Write "}" & vbNewLine
		...
		Response.Write "<FORM NAME=""ConceptValuesFrm"" ID=""ConceptValuesFrm"" ACTION=""" & sAction & """ METHOD=""POST"" onSubmit=""return CheckConceptValuesFields(this)"">"
			...
			If iSelectedTab <> -1 Then
				If Len(oRequest("ConceptValuesAction").Item) > 0 Then
					Response.Write "<INPUT TYPE=""HIDDEN"" NAME=""ConceptValuesAction"" ID=""SaveConceptValueHdn"" VALUE=""1"" />"
					If Len(oRequest("ChangeEndDate").Item) > 0 Then
						Response.Write "<INPUT TYPE=""HIDDEN"" NAME=""ChangeEndDate"" ID=""ChangeEndDateHdn"" VALUE=""1"" />"
						'Response.Write "<INPUT TYPE=""HIDDEN"" NAME=""RecordID"" ID=""RecordIDHdn"" VALUE=""" & oRequest("RecordID").Item & """ />"
					End If
				Else
					If Len(oRequest("ChangeEndDate").Item) > 0 Then
						Response.Write "<INPUT TYPE=""HIDDEN"" NAME=""ChangeEndDate"" ID=""ChangeEndDateHdn"" VALUE=""1"" />"
			...
			Response.Write "<TABLE BORDER=""0"" CELLPADDING=""0"" CELLSPACING=""0"">"
				If Len(oRequest("ChangeEndDate").Item) = 0 Then
					...
					Else
						Response.Write "<TR>"
							Response.Write "<TD><FONT FACE=""Arial"" SIZE=""2"">Concepto de pago:&nbsp;</FONT></TD>"
							If aConceptComponent(N_ID_CONCEPT) = -1 Then
					...
					Response.Write "<TR>"
						...
						If aConceptComponent(N_RECORD_ID_CONCEPT) = -1 Then
							...
							ElseIf aConceptComponent(N_EMPLOYEE_TYPE_ID_CONCEPT) = 1 Then
								Response.Write "<TD><SELECT NAME=""PositionIDTemp"" ID=""PositionIDTempCmb"" SIZE=""1"" CLASS=""Lists"" onChange=""var asTemp = this.value.split(','); document.ConceptValuesFrm.PositionID.value = asTemp[0]; document.ConceptValuesFrm.GroupGradeLevelID.value = asTemp[1]; document.ConceptValuesFrm.ClassificationID.value = asTemp[2]; document.ConceptValuesFrm.IntegrationID.value = asTemp[3]; document.ConceptValuesFrm.ClassificationIDTemp.value = asTemp[2]; document.ConceptValuesFrm.IntegrationIDTemp.value = asTemp[3]; document.ConceptValuesFrm.GroupGradeLevelIDTemp.value = asTemp[4]; document.ConceptValuesFrm.CompanyIDTemp.value = asTemp[5]; document.ConceptValuesFrm.PositionShortNameTemp.value = asTemp[6]; document.ConceptValuesFrm.PositionNameTemp.value = asTemp[7]; document.ConceptValuesFrm.CompanyID.value = asTemp[8];"">"
									'Response.Write "<OPTION VALUE=""-1,-1,-1,-1,-1,-1,-1,-1"">Todos</OPTION>"
									If bFull Then
										Response.Write "<OPTION VALUE=""-1,-1,-1,-1,-1,-1,-1,-1,-1"">Todos</OPTION>"
                                    Else
                                        Response.Write "<OPTION VALUE=""-1,-1,-1,-1,-1,-1,-1,-1,-1""> </OPTION>"
									End If
									Response.Write GenerateListOptionsFromQuery(oADODBConnection, "Positions, GroupGradeLevels, Companies", "PositionID, Positions.GroupGradeLevelID, ClassificationID, IntegrationID, GroupGradeLevelShortName, CompanyName, PositionShortName As ShortName, PositionName As Name, Positions.CompanyID", "PositionShortName, PositionName, 'GGN:' As Temp1, GroupGradeLevelShortName, 'Clas:' As Temp2, ClassificationID, 'Int:' As Temp3, IntegrationID, 'Cia:' As Temp, Positions.CompanyID", "(Positions.GroupGradeLevelID=GroupGradeLevels.GroupGradeLevelID) And (PositionID>-1) And (EmployeeTypeID=" & aConceptComponent(N_EMPLOYEE_TYPE_ID_CONCEPT) & ") And  (Positions.CompanyID = Companies.CompanyID)", "PositionShortName", aConceptComponent(N_POSITION_ID_CONCEPT), "", sErrorDescription)
								Response.Write "</SELECT></TD>"
                                ...
                                Response.Write "<INPUT TYPE=""HIDDEN"" NAME=""CompanyID"" ID=""CompanyIDHdn"" VALUE=""" & aConceptComponent(N_COMPANY_ID_CONCEPT) & """ />"
							Else
								Response.Write "<TD><SELECT NAME=""PositionIDTemp"" ID=""PositionIDTempCmb"" SIZE=""1"" CLASS=""Lists"" onChange=""var asTemp = this.value.split(','); document.ConceptValuesFrm.PositionID.value = asTemp[0]; document.ConceptValuesFrm.LevelID.value = asTemp[1]; document.ConceptValuesFrm.LevelIDTemp.value = asTemp[2]; document.ConceptValuesFrm.CompanyIDTemp.value = asTemp[3]; document.ConceptValuesFrm.PositionShortNameTemp.value = asTemp[4]; document.ConceptValuesFrm.PositionNameTemp.value = asTemp[5]; document.ConceptValuesFrm.CompanyID.value = asTemp[6];"">"
									If bFull Then
										Response.Write "<OPTION VALUE=""-1,-1,-1,-1,-1,-1,-1"">Todos</OPTION>"
                                    Else
                                        Response.Write "<OPTION VALUE=""-1,-1,-1,-1,-1,-1,-1""> </OPTION>"
									End If
									Response.Write GenerateListOptionsFromQuery(oADODBConnection, "Positions, Levels, Companies", "PositionID, Positions.LevelID, LevelName, CompanyName, PositionShortName As ShortName, PositionName As Name, Positions.CompanyID", "PositionShortName, PositionName, 'Nivel:' As Temp, Positions.LevelID, 'Cia:' As Temp, Positions.CompanyID", "(PositionID>-1) And (EmployeeTypeID=" & aConceptComponent(N_EMPLOYEE_TYPE_ID_CONCEPT) & ") And (Positions.LevelID=Levels.LevelID) And (Positions.CompanyID=Companies.CompanyID)", "PositionShortName", aConceptComponent(N_POSITION_ID_CONCEPT) & "," & aConceptComponent(N_LEVEL_ID_CONCEPT), "", sErrorDescription)
								Response.Write "</SELECT>"
								Response.Write "<INPUT TYPE=""HIDDEN"" NAME=""PositionID"" ID=""PositionIDHdn"" VALUE=""" & aConceptComponent(N_POSITION_ID_CONCEPT) & """ />"
					...
					If aConceptComponent(N_EMPLOYEE_TYPE_ID_CONCEPT) = 1 Then
						If aConceptComponent(N_RECORD_ID_CONCEPT) = -1 Then
							Response.Write "<TR>"
								Response.Write "<TD></TD><TD><FONT FACE=""Arial"" SIZE=""2"">Código:&nbsp;<INPUT TYPE=""TEXT"" NAME=""PositionShortNameTemp"" ID=""PositionShortNameTempTxt"" SIZE=""10"" MAXLENGTH=""10"" CLASS=""TextFields"" DISABLED=""1"" />"
								Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""10"" HEIGHT=""30"" ALIGN=""ABSMIDDLE"" /><FONT FACE=""Arial"" SIZE=""2"">Denominación:&nbsp;<INPUT TYPE=""TEXT"" NAME=""PositionNameTemp"" ID=""PositionNameTempTxt"" SIZE=""50"" MAXLENGTH=""50"" CLASS=""TextFields"" DISABLED=""1"" />"
								Response.Write "</TD>"
							Response.Write "</TR>"
							Response.Write "<TR>"
								Response.Write "<TD></TD><TD><FONT FACE=""Arial"" SIZE=""2"">Grupo, grado, nivel:&nbsp;<INPUT TYPE=""TEXT"" NAME=""GroupGradeLevelIDTemp"" ID=""GroupGradeLevelIDTempTxt"" SIZE=""3"" MAXLENGTH=""3"" CLASS=""TextFields"" DISABLED=""1"" />"
								Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""10"" HEIGHT=""30"" ALIGN=""ABSMIDDLE"" /><FONT FACE=""Arial"" SIZE=""2"">Clasificación:&nbsp;<INPUT TYPE=""TEXT"" NAME=""ClassificationIDTemp"" ID=""ClassificationIDTempTxt"" SIZE=""3"" MAXLENGTH=""3"" CLASS=""TextFields"" DISABLED=""1"" />"
								Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""10"" HEIGHT=""30"" ALIGN=""ABSMIDDLE"" /><FONT FACE=""Arial"" SIZE=""2"">Integración:&nbsp;<INPUT TYPE=""TEXT"" NAME=""IntegrationIDTemp"" ID=""IntegrationIDTempTxt"" SIZE=""3"" MAXLENGTH=""3"" CLASS=""TextFields"" DISABLED=""1"" />"
								Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""10"" HEIGHT=""30"" ALIGN=""ABSMIDDLE"" /><FONT FACE=""Arial"" SIZE=""2"">Empresa:&nbsp;<INPUT TYPE=""TEXT"" NAME=""CompanyIDTemp"" ID=""CompanyIDTempTxt"" SIZE=""20"" MAXLENGTH=""20"" CLASS=""TextFields"" DISABLED=""1"" />"
								Response.Write "</TD>"
							Response.Write "</TR>"
						End If
					End If
					If (InStr(1, ",0,2,3,4,5,6,", ("," & iSelectedTab & ","), vbBinaryCompare) > 0) Or bFull Then
						If aConceptComponent(N_EMPLOYEE_TYPE_ID_CONCEPT) <> 1 Then
							Response.Write "<TR>"
								...
								Else
									...
									If aConceptComponent(N_RECORD_ID_CONCEPT) = -1 Then
							            ...
							            Response.Write "</TR>"
							            Response.Write "<TR>"
										    Response.Write "<TD></TD><TD><FONT FACE=""Arial"" SIZE=""2"">Nivel:&nbsp;<INPUT TYPE=""TEXT"" NAME=""LevelIDTemp"" ID=""LevelIDTempTxt"" SIZE=""3"" MAXLENGTH=""3"" CLASS=""TextFields"" DISABLED=""1"" />"
										    Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""10"" HEIGHT=""30"" ALIGN=""ABSMIDDLE"" /><FONT FACE=""Arial"" SIZE=""2"">Empresa:&nbsp;<INPUT TYPE=""TEXT"" NAME=""CompanyIDTemp"" ID=""CompanyIDTempTxt"" SIZE=""20"" MAXLENGTH=""20"" CLASS=""TextFields"" DISABLED=""1"" />"
										    Response.Write "</TD>"
							            Response.Write "</TR>"
									Else
				...
				If Len(oRequest("ChangeEndDate").Item) > 0 Then
					Response.Write "<TR>"
						...
						Response.Write "<TD><FONT FACE=""Arial"" SIZE=""2"">" & DisplayDateFromSerialNumber(aConceptComponent(N_START_DATE_FOR_VALUE_CONCEPT), -1, -1, -1) & "</FONT></TD>"
				...
				End If
				Response.Write "<TR>"
					Response.Write "<TD><FONT FACE=""Arial"" SIZE=""2"">Fin de vigencia:&nbsp;</FONT></TD>"
					Response.Write "<TD><FONT FACE=""Arial"" SIZE=""2"">"
						Response.Write DisplayDateCombosUsingSerial(aConceptComponent(N_END_DATE_FOR_VALUE_CONCEPT), "EndForValue", Year(Date) - 10, Year(Date) + 1, True, True)
					Response.Write "</FONT></TD>"
				Response.Write "</TR>"
				If bFull Then
					Response.Write "<TR>"
			...
			If Len(oRequest("ChangeEndDate").Item) = 0 Then
				...
				End If
				Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""100"" HEIGHT=""1"" />"
				Response.Write "<INPUT TYPE=""BUTTON"" NAME=""Cancel"" ID=""CancelBtn"" VALUE=""Cancelar"" CLASS=""Buttons"" onClick=""window.location.href='" & GetASPFileName("") & "?Action=" & oRequest("Action").Item & "&EmployeeTypeID=" & oRequest("EmployeeTypeID").Item & "&ConceptID=" & aConceptComponent(N_ID_CONCEPT) & "'"" />"
				Response.Write "<BR /><BR />"
			Else
				Response.Write "<INPUT TYPE=""HIDDEN"" NAME=""EmployeeTypeID"" ID=""EmployeeTypeIDHdn"" VALUE=""" & oRequest("EmployeeTypeID").Item & """ />"
				Response.Write "<INPUT TYPE=""SUBMIT"" NAME=""ChangeEndDateButton"" ID=""ChangeEndDateBtn"" VALUE=""Modificar Fecha Fin"" CLASS=""Buttons"" />"
				Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""100"" HEIGHT=""1"" />"
				Response.Write "<INPUT TYPE=""BUTTON"" NAME=""Cancel"" ID=""CancelBtn"" VALUE=""Cancelar"" CLASS=""Buttons"" onClick=""window.location.href='" & GetASPFileName("") & "?Action=" & oRequest("Action").Item & "&EmployeeTypeID=" & oRequest("EmployeeTypeID").Item & "'"" />"
				Response.Write "<BR /><BR />"
			End If
			Call DisplayWarningDiv("RemoveConceptWngDiv", "¿Está seguro que desea borrar el registro de la base de datos?")
		Response.Write "</FORM>"
