...
'************************************************************
	...
	If aEmployeeComponent(N_ID_EMPLOYEE) <> -1 Then
		lErrorNumber = GetEmployee(oRequest, oADODBConnection, aEmployeeComponent, sErrorDescription)
	End If
	strRutaFoto  = CStr(Server.MapPath("Uploaded Files\e"&aEmployeeComponent(N_ID_EMPLOYEE)&"\" & aEmployeeComponent(S_NUMBER_EMPLOYEE) & ".jpg"))
			...
			'If Len(aEmployeeComponent(S_ADDRESS_EMPLOYEE))>0 Then
				Response.Write "<TABLE>"
					...
					Response.Write "<TR>"
						...
						Response.Write "<TD><FONT FACE=""Arial"" SIZE=""1""><b>EXT.</b></FONT></TD>"
				...
				If lErrorNumber = 0 Then
					...
					If lErrorNumber = 0 Then
						...
						If lErrorNumber = 0 Then
							...
							End If
							If (lReasonID = 12 Or lReasonID = 13 Or lReasonID = 17 Or lReasonID = 18 Or lReasonID = 28) And (aEmployeeComponent(N_EMPLOYEE_TYPE_ID_EMPLOYEE) = 7) Then
								bActivate = False
						...
						If lErrorNumber = 0 Then
							...
							End If
							If lErrorNumber <> 0 Then
								If (aEmployeeComponent(N_EMPLOYEE_TYPE_ID_EMPLOYEE) = 7) Or _
	...
	If lErrorNumber = 0 Then
		If lErrorNumber = 0 Then
			...
			End If
				...
				End If
				If (aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_11_PERMISSIONS4) And (aEmployeeComponent(N_STATUS_ID_EMPLOYEE) <> 0) And (InStr(1, aEmployeeComponent(S_SECTIONS_TO_SHOW_EMPLOYEE), ",131,", vbBinaryCompare) = 0) Then
					If aEmployeeComponent(N_STATUS_REASON_ID_EMPLOYEE) = 2 Or aEmployeeComponent(N_STATUS_REASON_ID_EMPLOYEE) = 3 Then
				...
				If (InStr(1, sURL, ",EmployeeManagement,", vbBinaryCompare) = 0) Then
					...
					Else
						If bActivate Then
							...
							End If
							If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_13_PERMISSIONS4) = N_13_PERMISSIONS4) Or ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_02_PERMISSIONS4) = N_02_PERMISSIONS4) Then
								Response.Write "<INPUT TYPE=""SUBMIT"" NAME=""Register"" ID=""ModifyBtn"" VALUE=""Registrar para Validación"" CLASS=""Buttons"" />"
								Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""20"" HEIGHT=""1"" />"
							End If
							If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_13_PERMISSIONS4) = N_13_PERMISSIONS4) Then Response.Write "<INPUT TYPE=""SUBMIT"" NAME=""RemoveMotion"" ID=""ModifyBtn""    VALUE=""   Cancelar Movimiento   "" CLASS=""RedButtons"" />"
							Response.Write "<BR /><BR />"
							If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_11_PERMISSIONS4) = N_11_PERMISSIONS4) Then Response.Write "<INPUT TYPE=""SUBMIT"" NAME=""Validate"" ID=""ModifyBtn"" VALUE=""Registrar para Autorización"" CLASS=""Buttons"" />"
							Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""20"" HEIGHT=""1"" />"
								Response.Write "<DIV NAME=""ApplyMovementDiv"" ID=""ApplyMovementDiv"" STYLE=""display: none"">"
									If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_11_PERMISSIONS4) = N_11_PERMISSIONS4) Then Response.Write "<INPUT TYPE=""SUBMIT"" NAME=""Authorization"" ID=""ModifyBtn"" VALUE=""     Aplicar Movimiento    "" CLASS=""RedButtons"" onClick=""bCheckReason=false""/>"
								Response.Write "</DIV>"
							If (lReasonID > 0) Or (lReasonID = -64) Or (lReasonID = -75) Then
								Response.Write "<SCRIPT LANGUAGE=""JavaScript""><!--" & vbNewLine
						...
						Else
							Select Case aEmployeeComponent(N_STATUS_REASON_ID_EMPLOYEE)
								Case 1
									'If Not aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_11_PERMISSIONS4 Then
									If Not ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_11_PERMISSIONS4) = N_11_PERMISSIONS4) Then
										If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_13_PERMISSIONS4) = N_13_PERMISSIONS4) Then
											Response.Write "<INPUT TYPE=""SUBMIT"" NAME=""Register"" ID=""ModifyBtn"" VALUE=""  Registrar para Validación  "" CLASS=""Buttons"" />"
											Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""20"" HEIGHT=""1"" />"
										End If
										If aLoginComponent(N_USER_PERMISSIONS_LOGIN) And N_MODIFY_PERMISSIONS Then
									...
									If (InStr(1, sStatusEmployeesIDs1, aEmployeeComponent(N_STATUS_ID_EMPLOYEE), vbBinaryCompare) > 0) Then
										If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_12_PERMISSIONS4) = N_12_PERMISSIONS4) Then
										...
										End If
										Response.Write "<DIV NAME=""ApplyMovementDiv"" ID=""ApplyMovementDiv"" STYLE=""display: none"">"
											If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_12_PERMISSIONS4) = N_12_PERMISSIONS4) Then Response.Write "<INPUT TYPE=""SUBMIT"" NAME=""Authorization"" ID=""ModifyBtn"" VALUE=""    Aplicar Movimiento     "" CLASS=""RedButtons"" onClick=""bCheckReason=false""/>"
										Response.Write "</DIV>"
										If lReasonID > 0 Then
											Response.Write "<SCRIPT LANGUAGE=""JavaScript""><!--" & vbNewLine
												Response.Write "ShowHideApplyMovementButton(document.EmployeeFrm.EmployeePayrollDate.value);" & vbNewLine
								...
								Case 2
									...
									If (InStr(1, sStatusEmployeeIDs1, aEmployeeComponent(N_STATUS_ID_EMPLOYEE), vbBinaryCompare) > 0) Then
										If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_11_PERMISSIONS4) = N_11_PERMISSIONS4) Then
											Response.Write "<INPUT TYPE=""SUBMIT"" NAME=""CancelMotion"" ID=""ModifyBtn"" VALUE=""Rechazar Movimiento"" CLASS=""Buttons"" onClick=""bCheckReason=true""/>"
											Response.Write "<IMG SRC=""Images/Transparent.gif"" WIDTH=""20"" HEIGHT=""1"" />"
										End If
										If ((aLoginComponent(N_USER_PERMISSIONS4_LOGIN) And N_11_PERMISSIONS4) = N_11_PERMISSIONS4) Then
		...
		Select Case lReasonID
			...
			Case EMPLOYEES_GLASSES
				Response.Write "if (oForm.ConceptAmount.value<=0) {" & vbNewLine
					...
					Response.Write "return false;" & vbNewLine
				Response.Write "}" & vbNewLine
				Response.Write "if (oForm.ConceptAmount.value>1400) {" & vbNewLine
					Response.Write "alert('El valor del Importe para ayuda de anteojos no puede ser mayor a 1400 pesos.');" & vbNewLine
					Response.Write "return false;" & vbNewLine
				Response.Write "}" & vbNewLine
			Case Else
				Response.Write "if (oForm.ConceptAmount.value<=0) {" & vbNewLine
...
'************************************************************
	...
	Response.Write "<TABLE BORDER=""0"" CELLPADDING=""0"" CELLSPACING=""0"">"
		If (aEmployeeComponent(N_EMPLOYEE_TYPE_ID_EMPLOYEE) = 7) Then
			...
			Response.Write "<TR>"
				Response.Write "<TD><FONT FACE=""Arial"" SIZE=""2"">Salario:&nbsp;</FONT></TD>"
	...
	Response.Write "<TABLE BORDER=""0"" CELLPADDING=""0"" CELLSPACING=""0"">"
		...
		Response.Write "<TR>"
			...
			Select Case lReasonID
				...
				Case -89, EMPLOYEES_ADDITIONALSHIFT, EMPLOYEES_GLASSES, EMPLOYEES_FAMILY_DEATH, EMPLOYEES_PROFESSIONAL_DEGREE, EMPLOYEES_LICENSES, EMPLOYEES_CONCEPT_16, EMPLOYEES_NON_EXCENT, EMPLOYEES_EXCENT, EMPLOYEES_ANUAL_AWARD, EMPLOYEES_MONTHAWARD, EMPLOYEES_SPORTS, EMPLOYEES_CARLOAN, EMPLOYEES_CONCEPT_C3, EMPLOYEES_LICENSES, EMPLOYEES_CONCEPT_16
			...
			Response.Write "<TD>"
				...
				Select Case lReasonID
					...
					Case EMPLOYEES_SAFE_SEPARATION, 53, EMPLOYEES_CONCEPT_08, EMPLOYEES_ADDITIONALSHIFT, EMPLOYEES_CONCEPT_7S
						...
						Response.Write "</SPAN>"
					' Dinero ($)
					Case -89, EMPLOYEES_ANTIQUITIES, EMPLOYEES_GLASSES, EMPLOYEES_FAMILY_DEATH, EMPLOYEES_PROFESSIONAL_DEGREE, EMPLOYEES_LICENSES, EMPLOYEES_CONCEPT_16, EMPLOYEES_NON_EXCENT, EMPLOYEES_EXCENT, EMPLOYEES_ANUAL_AWARD, EMPLOYEES_MONTHAWARD, EMPLOYEES_SPORTS, EMPLOYEES_CARLOAN, EMPLOYEES_CONCEPT_C3, EMPLOYEES_LICENSES, EMPLOYEES_CHILDREN_SCHOOLARSHIPS, EMPLOYEES_BENEFICIARIES_DEBIT, EMPLOYEES_FONAC_ADJUSTMENT
						Response.Write "<SELECT NAME=""ConceptQttyID"" ID=""ConceptQttyIDCmb"" SIZE=""1"" CLASS=""Lists"" onChange=""ShowAmountFields(this.value, 'Concept');"">"
