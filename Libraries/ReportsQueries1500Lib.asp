...
'************************************************************
	...
	If lErrorNumber = 0 Then
		If Not oRecordset.EOF Then
			...
			asCellWidths = Split("80,200,100,150,150,60,100,100,300,400", ",", -1, vbBinaryCompare)
            ...
            lErrorNumber = AppendTextToFile(sFilePath, "<TABLE BORDER=""1"" CELLSPACING=""0"" CELLPADDING=""0"">", sErrorDescription)
                asColumnsTitles = Split("=T(""" & sFolio & """);;;" & sResponsable & ";;;NA;;;" & sFechaDoc & ";;;" & sFechaLimite & ";;;Ven;;;" & sTipoAsunto & ";;;" & sEstatus & ";;;Asunto;;;Comentarios", LIST_SEPARATOR, -1, vbBinaryCompare)
				...
				asColumnsTitles = Split("Folio;;;Area;;;Digitalizado;;;F. del documento;;;F. límite;;;Ven;;;T. de Asunto;;;Estatus / Prioridad;;;Asunto;;;Comentarios", LIST_SEPARATOR, -1, vbBinaryCompare)
				lErrorNumber = AppendTextToFile(sFilePath, GetTableHeaderPlainText(asColumnsTitles, True, sErrorDescription), sErrorDescription)
				...
				Do While Not oRecordset.EOF
					...
					sComentarios = ""
                    If CLng(oRecordset.Fields("EndDate").Value) <> 0 Then
						sFechaCierre = DisplayDateFromSerialNumber(CLng(oRecordset.Fields("EndDate").Value), -1, -1, -1)
					End If
					If CInt(oRecordset.Fields("OwnerID").Value) = 1000 Or CInt(oRecordset.Fields("OwnerID").Value) = 1100 Or CInt(oRecordset.Fields("OwnerID").Value) = 1200 Or CInt(oRecordset.Fields("OwnerID").Value) = 1300 Or CInt(oRecordset.Fields("OwnerID").Value) = 1400 Then
						If Len(oRecordset.Fields("ClosingNumber").Value) > 1 Then
							sExtraInfo = CStr(oRecordset.Fields("ClosingNumber").Value)
						End If
						If Len(oRecordset.Fields("Comenta").Value) > 1 Then
							sExtraInfo = sExtraInfo & " " & CStr(oRecordset.Fields("Comenta").Value)
						End If
					End If
					sRowContents = "=T(""" & CleanStringForHTML(CStr(oRecordset.Fields("PaperworkNumber").Value)) & """)"
					...
					If (ExistPaperworkFiles(CLng(Left(oRecordset.Fields("StartDate").Value, Len("0000"))), CStr(oRecordset.Fields("PaperworkNumber").Value))) Then
                        sRowContents = sRowContents & TABLE_SEPARATOR & "SI"
					Else
                        sRowContents = sRowContents & TABLE_SEPARATOR & "NO"
					End If
                    sRowContents = sRowContents & TABLE_SEPARATOR & DisplayDateFromSerialNumber(CLng(oRecordset.Fields("StartDate").Value), -1, -1, -1) & " " & CleanStringForHTML(CStr(oRecordset.Fields("DocumentNumber").Value)) & " " & sFechaCierre & "  " & sExtraInfo
                    ...
                    End If
                    sRowContents = sRowContents & TABLE_SEPARATOR & CleanStringForHTML(CStr(oRecordset.Fields("SubjectTypeName").Value))
					sRowContents = sRowContents & TABLE_SEPARATOR & CleanStringForHTML(CStr(oRecordset.Fields("StatusName").Value))  & " / " & CleanStringForHTML(CStr(oRecordset.Fields("PriorityName").Value))
                    'If CLng(oRecordset.Fields("EndDate").Value) = 0 Then
					'	sRowContents = sRowContents & TABLE_SEPARATOR & CleanStringForHTML(CStr(oRecordset.Fields("StatusName").Value))
					'Else
					'	sRowContents = sRowContents & TABLE_SEPARATOR & "Cerrado"
					'End If
					sRowContents = sRowContents & TABLE_SEPARATOR & CleanStringForReportField(CStr(oRecordset.Fields("DocumentSubject").Value))
					Call GetPpwkComments(oADODBConnection, CLng(oRecordset.Fields("PaperworkID").Value), sComentarios, sErrorDescription)
					sRowContents = sRowContents & TABLE_SEPARATOR & CleanStringForReportField(sComentarios)
					asRowContents = Split(sRowContents, TABLE_SEPARATOR, -1, vbBinaryCompare)
					lErrorNumber = AppendTextToFile(sFilePath, GetTableRowText(asRowContents, True, sErrorDescription), sErrorDescription)
					oRecordset.MoveNext
					If (Err.number <> 0) Or (lErrorNumber <> 0) Then Exit Do
