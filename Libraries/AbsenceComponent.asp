			...
			Select Case aAbsenceComponent(N_ABSENCE_ID_ABSENCE)
				Case 29, 30, 35, 37, 38
					...
					If aAbsenceComponent(N_HOURS_ABSENCE) = 0 Then
						lErrorNumber = -1
						sErrorDescription = "En las fechas introducidas el empleado no tiene días laborables que sean considerados para el registro de la incidencia seleccionada."
					End If
				Case Else
...
'************************************************************
	...
	Else
		...
		Else
			...
			lErrorNumber = ExecuteSQLQuery(oADODBConnection, "Update EmployeesAbsencesLKP Set DocumentNumber='" & aAbsenceComponent(S_DOCUMENT_NUMBER_ABSENCE) & "', JustificationID=" & aAbsenceComponent(N_ABSENCE_ID_ABSENCE) & ", AppliesForPunctuality=" & aAbsenceComponent(N_APPLIES_FOR_PUNCTUALITY_ABSENCE) & ", Removed=1, RemoveUserID=" & aAbsenceComponent(N_REMOVE_USER_ID_ABSENCE) & ", RemovedDate=" & Left(GetSerialNumberForDate(""), Len("00000000")) & ", AppliedRemoveDate=" & aAbsenceComponent(N_APPLIED_REMOVE_DATE_ABSENCE) & ", Active=" & aAbsenceComponent(N_ACTIVE_ABSENCE) & " Where (EmployeeID=" & aAbsenceComponent(N_EMPLOYEE_ID_ABSENCE) & ") And (AbsenceID=" & iAbsenceID & ") And (OcurredDate=" & aAbsenceComponent(N_OCURRED_DATE_ABSENCE) & ")", "AbsenceComponent.asp", S_FUNCTION_NAME, 000, sErrorDescription, Null)
		End If
	End If
...
'************************************************************
	...
	sQuery = "Update EmployeesAbsencesLKP set Active = 1" & _
		" where (Active = 0)"
			...
			Response.Write "function ClearOcurredDates() {" & vbNewLine
				...
				Response.Write "else {" & vbNewLine
					...
					Response.Write "else {" & vbNewLine
						...
						Response.Write "}" & vbNewLine
						Response.Write "else {" & vbNewLine
							Response.Write "HideDisplay(document.all['AbsencesForPeriod1Div'])" & vbNewLine
						Response.Write "}" & vbNewLine
					Response.Write "}" & vbNewLine
					...
					If lErrorNumber = 0 Then
						...
						If lErrorNumber = 0 Then
							...
							Response.Write "<DIV NAME=""AbsencesForPeriod1Div"" ID=""AbsencesForPeriod1Div"" STYLE=""display: none"">"
								Response.Write "<FORM NAME=""UploadValidateInfoFrm"" ID=""UploadValidateInfoFrm"" METHOD=""POST"" onSubmit=""return bReady"">"
									Call DisplayErrorMessage("Advertencia", "Para registrar periodos de incidencias a los empleados con tipo de jornada 2 debe de registrar los días de forma individual, ya sean salteados o continuos.")
								Response.Write "</FORM>"
							Response.Write "</DIV>"
							Response.Write "<FORM NAME=""AbsencesFrm"" ID=""AbsencesFrm"" ACTION=""" & sAction & """ METHOD=""GET"" onSubmit=""return CheckAbsenceFields(this)"">"
													...
													Response.Write "<TABLE BORDER=""0"" CELLPADDING=""0"" CELLSPACING=""0"">"
															Response.Write "<TR><BR />"
																Response.Write "<TD VALIGN=""TOP"">"
																	Response.Write "<IFRAME SRC=""BrowserMonthForPayments.asp?EmployeeID=" & aAbsenceComponent(N_EMPLOYEE_ID_ABSENCE) & "&EmployeeDate=" & aAbsenceComponent(N_OCURRED_DATE_ABSENCE) & "&JourneyID=" & aEmployeeComponent(N_JOURNEY_ID_EMPLOYEE) & "&FromArrow=1&ReasonID=1&" & sExtraURL & """ NAME=""BrowserMonthIFrame"" FRAMEBORDER=""0"" WIDTH=""330"" HEIGHT=""130""></IFRAME>"
																Response.Write "</TD>"
...
'*****************************************************************
	...
	sQuery = "Select EA.EmployeeID, EA.AbsenceID, EmployeeName + ' ' + EmployeeLastName + ' ' + EmployeeLastName2 As EmployeeFullName," &_
			...
			" A.AbsenceName, EA.VacationPeriod, J.JustificationShortName, EA.Reasons, EA.Removed, EA.AppliedDate," & _
	...
	If lErrorNumber = 0 Then
		If Not oRecordset.EOF Then
			...
			If bForExport Then
				If (lReasonID = 0) Then
					If (Len(oRequest("ShortFormat").Item) > 0) Then
						asColumnsTitles = Split("Empleado, F. inicio, F. término, F. de registro, Clave, Descripción, Periodo, Justificada", ",", -1, vbBinaryCompare)
					Else
						asColumnsTitles = Split("No. Empleado, Nombre, Fecha de Ocurrencia, Fecha de término, Fecha de registro, Documento, Días, Clave, Descripción, Periodo, Justificada, Observaciones", ",", -1, vbBinaryCompare)
					End If
				Else
					If (Len(oRequest("ShortFormat").Item) > 0) Then
						asColumnsTitles = Split("Empleado, F. inicio, F. término, F. de registro, Clave, Descripción, Periodo, Justificada", ",", -1, vbBinaryCompare)
			...
			Else
				If (lReasonID = 0) Then
					asColumnsTitles = Split("No. Empleado, Nombre, Fecha de Ocurrencia, Fecha de término, Fecha de registro, Documento, Días, Clave, Descripción, Periodo, Justificada, Observaciones, Acciones", ",", -1, vbBinaryCompare)
				Else
					asColumnsTitles = Split("No. Empleado, Nombre, Fecha de inicio, Fecha de termino, Fecha de registro, Documento, Días, Clave, Descripción, Periodo, Justificada, Observaciones, Acciones", ",", -1, vbBinaryCompare)
				End If
				asCellWidths = Split(",,,,,,,,,,,",",", -1, vbBinaryCompare)
				asCellAlignments = Split("CENTER,,,,,,,CENTER", ",", -1, vbBinaryCompare)
			End If
			...
			Do While Not oRecordset.EOF
				...
				If (CLng(oRecordset.Fields("AbsenceID").Value) = 35) Or (CLng(oRecordset.Fields("AbsenceID").Value) = 37) Or (CLng(oRecordset.Fields("AbsenceID").Value) = 38) Or (CLng(oRecordset.Fields("AbsenceID").Value) = 39) Or (CLng(oRecordset.Fields("AbsenceID").Value) = 40) Then
					If (CLng(oRecordset.Fields("AbsenceID").Value) = 39) Or (CLng(oRecordset.Fields("AbsenceID").Value) = 40) Then
						sRowContents = sRowContents & TABLE_SEPARATOR & CleanStringForHTML(Left(CStr(oRecordset.Fields("VacationPeriod").Value), 4) & "-" & Right(CStr(oRecordset.Fields("VacationPeriod").Value), 2))
					Else
						sRowContents = sRowContents & TABLE_SEPARATOR & CleanStringForHTML(Left(CStr(oRecordset.Fields("VacationPeriod").Value), 4) & "-" & Right(CStr(oRecordset.Fields("VacationPeriod").Value), 1))
					End If
				Else
					sRowContents = sRowContents & TABLE_SEPARATOR & CleanStringForHTML("NA")
				End If
		...
		If bIsForPeriod Then
			...
			Else
				...
				If InStr(1, ",50,51,52,53,54,55,56,", "," & CStr(aAbsenceComponent(N_ABSENCE_ID_ABSENCE)) & ",", vbBinaryCompare) = 0 Then
					sCondition = sCondition & " And (AbsenceID NOT In (201,202,39,50,51,52,53,54,55,56))"
				Else
					sCondition = sCondition & " And (AbsenceID NOT In (201,202))"
				End If
...
'************************************************************
	...
	Dim lNextStartDateForVacations	' Siguiente día habíl para registrar las vacaciones
	Dim lNextEndDateForVacations	' Siguiente día habíl para registrar el fin de las vacaciones que se truncan
