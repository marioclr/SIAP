...
Err.Clear
sFilePath = Server.MapPath("Export\aPayroll_" & aLoginComponent(N_USER_ID_LOGIN) & "_20130131_Proc")
Set oADODBCommand = Server.CreateObject("ADODB.Command")
Set oADODBCommand.ActiveConnection = oADODBConnection
oADODBCommand.commandtype=4
...
oADODBCommand("lForPayrollID") = 20130131
oADODBCommand("iPayrollType") = 4
oADODBCommand("sConditions") = ""

Call AppendTextToFile(sFilePath & "_Rastros.txt", "Antes" & vbTab & "Err.number=" & Err.number & vbTab & "Err.description=" & Err.description & vbTab & "TimeStamp=" & Year(Now()) & Right(("0" & Month(Now())), Len("00")) & Right(("0" & Day(Now())), Len("00")) & Right(("0" & Hour(Now())), Len("00")) & Right(("0" & Minute(Now())), Len("00")) & Right(("0" & Second(Now())), Len("00")) & vbTab & "bTimeout=" & bTimeout, "Registro de Rastros")
oADODBCommand.Execute
Call AppendTextToFile(sFilePath & "_Rastros.txt", "Después" & vbTab & "Err.number=" & Err.number & vbTab & "Err.description=" & Err.description & vbTab & "TimeStamp=" & Year(Now()) & Right(("0" & Month(Now())), Len("00")) & Right(("0" & Day(Now())), Len("00")) & Right(("0" & Hour(Now())), Len("00")) & Right(("0" & Minute(Now())), Len("00")) & Right(("0" & Second(Now())), Len("00")) & vbTab & "bTimeout=" & bTimeout, "Registro de Rastros")

lTotal = oADODBCommand("iInsertCounts")
Call AppendTextToFile(sFilePath & "_Rastros.txt", "iInsertCounts" & lTotal & vbTab & "Err.number=" & Err.number & vbTab & "Err.description=" & Err.description & vbTab & "TimeStamp=" & Year(Now()) & Right(("0" & Month(Now())), Len("00")) & Right(("0" & Day(Now())), Len("00")) & Right(("0" & Hour(Now())), Len("00")) & Right(("0" & Minute(Now())), Len("00")) & Right(("0" & Second(Now())), Len("00")) & vbTab & "bTimeout=" & bTimeout, "Registro de Rastros")

If (Err.number > 0) Then
	sFilePath = Server.MapPath("Export\Payroll_" & aLoginComponent(N_USER_ID_LOGIN) & "_RastreoExec")
End If

'lErrorNumber = ExecuteSQLQuery(oADODBConnection, "dbms_stats.gather_table_stats('SIAP','Payroll_20130131',ESTIMATE_PERCENT=>25,METHOD_OPT=>'for all indexed columns size auto',CASCADE=>True)", "PayrollComponentConstants.asp", "Payments.asp", 000, sErrorDescription, Null)
'Call AppendTextToFile(sFilePath & "_Rastros.txt", "dbms_stats.gather_table_stats('SIAP','Payroll_20130131',ESTIMATE_PERCENT=>25,METHOD_OPT=>'for all indexed columns size auto',CASCADE=>True)" & vbTab & "lErrorNumber=" & lErrorNumber & vbTab & "Err.number=" & Err.number & "Err.description=" & Err.description & vbTab & "TimeStamp=" & Year(Now()) & Right(("0" & Month(Now())), Len("00")) & Right(("0" & Day(Now())), Len("00")) & Right(("0" & Hour(Now())), Len("00")) & Right(("0" & Minute(Now())), Len("00")) & Right(("0" & Second(Now())), Len("00")) & vbTab & "bTimeout=" & bTimeout, "Registro de Rastros")
'oADODBConnection.Execute "dbms_stats.gather_table_stats('SIAP','Payroll_20130131',ESTIMATE_PERCENT=>25,METHOD_OPT=>'for all indexed columns size auto',CASCADE=>True)", -1, 4

lErrorNumber = RenameFileExtension(Server.MapPath(REPORTS_PATH) & "\Rep_1400_20130221160836.txt", ".txt", ".rtf", sErrorDescription)
