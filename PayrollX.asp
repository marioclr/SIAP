...
Select Case sAction
	...
	Case Else
		...
		If Len(oRequest("Add").Item) > 0 Then
			...
			If lErrorNumber = 0 Then
				bShowTable = True
				lErrorNumber = CalculatePayrol1(oRequest, oADODBConnection, 1, aPayrollComponent, sErrorDescription)
			End If
		ElseIf Len(oRequest("Modify").Item) > 0 Then
		...
		ElseIf Len(oRequest("CalculatePayroll").Item) > 0 Then
			...
			If Len(Application.Contents("SIAP_CalculatePayroll")) = 0 Then
				...
				lErrorNumber = CalculatePayrol1(oRequest, oADODBConnection, 2, aPayrollComponent, sErrorDescription)
			End If
			If lErrorNumber =  0 Then
		...
		ElseIf Len(oRequest("DoClose").Item) > 0 Then
			...
			Else
				lErrorNumber = CalculatePayrol1(oRequest, oADODBConnection, 3, aPayrollComponent, sErrorDescription)
				If lErrorNumber =  0 Then
					lErrorNumber = GetPayroll(oRequest, oADODBConnection, aPayrollComponent, sErrorDescription)
					If lErrorNumber =  0 Then
				...
				Select Case sAction
					...
					Case "ModifyPayroll"
						If Len(oRequest("CalculatePayroll").Item) > 0 Then
							...
							sNames = L_NO_INSTRUCTIONS_FLAGS & "," & L_OPEN_PAYROLL_FLAGS & "," & L_EMPLOYEE_NUMBER_FLAGS
