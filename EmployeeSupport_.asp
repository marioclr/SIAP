		...
		ElseIf Len(oRequest("Owners").Item) > 0 Then
			Response.Write "<FORM NAME=""OwnersFrm"" ID=""OwnersFrm"" ACTION=""EmployeeSupport.asp"" METHOD=""POST"">"
				...
				If Len(oRequest("UserID").Item) = 0 Then
					Call DisplayInstructionsMessage("Permisos de los usuarios para ver responsables", "<B>Paso 1. Seleccione al usuario</B><BR />Paso 2. Seleccione los responsables que podrá ver")
					Response.Write "<BR /><BR />"
					...
					Response.Write "<SELECT NAME=""UserID"" ID=""UserIDCmb"" SIZE=""1"" CLASS=""Lists"">"
						...
						lErrorNumber = ExecuteSQLQuery(oADODBConnection, "Select UserID, UserName, UserLastName, UserPermissions4, ProfileName From Users, UserProfiles Where (Users.ProfileID=UserProfiles.ProfileID) And (UserID>9) And ((UserPermissions4>=" & N_30_PERMISSIONS4 & ") Or (UserPermissions4=-1)) Order By UserLastName, UserName", "EmployeeSupport.asp", "_root", 000, sErrorDescription, oRecordset)
						If lErrorNumber = 0 Then
							Do While Not oRecordset.EOF
								If (CLng(oRecordset.Fields("UserPermissions4").Value) And N_30_PERMISSIONS4) = N_30_PERMISSIONS4 Then
				...
				Else
					Call DisplayInstructionsMessage("Permisos de los usuarios para ver responsables", "Paso 1. Seleccione al usuario<BR /><B>Paso 2. Seleccione los responsables que podrá ver</B>")
					Response.Write "<BR /><BR />"
...
'				End If
			...
			If CLng(aCatalogComponent(AS_FIELDS_VALUES_CATALOG)(0)) > -1 Then
				Response.Write "<IMG SRC=""Images/DotBlue.gif"" WIDTH=""700"" HEIGHT=""1"" /><BR /><BR />"
				Response.Write "<FONT FACE=""Arial"" SIZE=""2""><B>SEGUIMIENTO</B><BR /><BR /></FONT>"
