...
If Len(oRequest("OnlySundays").Item) > 0 Then
	sOnlySundays = "&OnlySundays=1"
ElseIf Len(oRequest("OnlyHolidays").Item) > 0 Then
	sOnlySundays = "&OnlyHolidays=1"
End If
...
End If
aCalendarComponent(S_TARGET_PAGE_CALENDAR) = "BrowserMonthForPayments.asp?FormName=" & sFormName & "&EmployeeID=" & aAbsenceComponent(N_EMPLOYEE_ID_ABSENCE) & sOnlySundays
	...
	<BODY TEXT="#000000" LINK="#000000" ALINK="#000000" VLINK="#000000" BGCOLOR="#FFFFFF" LEFTMARGIN="0" TOPMARGIN="0" MARGINWIDTH="0" MARGINHEIGHT="0">
		<TABLE WIDTH="1" BORDER="0" CELLPADDING="0" CELLSPACING="0"><TR>
			...
			ElseIf (iMonth = 2) And (iDay > 28) Then
				iDay = 28
			End If
			sURLForCalendar = RemoveParameterFromURLString(ReplaceValueInURLString(ReplaceValueInURLString(ReplaceValueInURLString(oRequest, "Year", iYear), "Month", iMonth), "Day", iDay), "EmployeeDate")
